<script>
    ...
    $: styles = (() => {
        ...
        // TODO: replace global properties
        debouncedValue = debouncedValue.replace("$user_name", "Сергей");
        if (split && activeTab === false) {
            if (sidebar) {
    ...
    onMount(async () => {
        ...
        const updateBlockPositions = throttle(() => {
            ...
            for (let i = 0; i < leftNodes.length; i++) {
                ...
                const left =
                    editor.heightAtLine(
                        leftNode.position.start.line - 1,
                        "local",
                    ) /
...
>
    ...
    <div class="bytemd-body">
        <div bind:this={editorEl} class="bytemd-editor" style={styles.edit} />
        <div
            bind:this={previewEl}
